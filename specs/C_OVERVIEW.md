...
**Columns** (Markets — extracted dynamically):
```javascript
// ✅ CORRECT: Extract markets from data, exclude aggregate
function getHeatmapMarkets(data) {
    return getUniqueDimensionValues(data, 'Market')
        .filter(m => m !== 'All Markets');
}

// Usage:
const markets = getHeatmapMarkets(state.museData);
// Renders columns for whatever markets exist in the data
```
